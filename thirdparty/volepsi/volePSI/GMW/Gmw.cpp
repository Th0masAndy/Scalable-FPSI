...
namespace volePSI {
    ...
    Proto Gmw::generateTriple(u64 batchSize, u64 numThreads, coproto::Socket &chl)
    {
    ...
    Proto Gmw::roundFunction(coproto::Socket &chl)
    {
        ...
        for (gate = gates.begin(); gate < gates.end(); ++gate) {
            ...
            //}
            if (mO.mDebug) {
                if (dirtyBits[gate->mInput[0]] || (dirtyBits[gate->mInput[1]] && gate->mType != oc::GateType::a)) {
            ...
            if (gate->mType == oc::GateType::a) {
                ...
                gate->mType == oc::GateType::na_And || gate->mType == oc::GateType::nb_And || gate->mType == oc::GateType::And ||
                gate->mType == oc::GateType::Nor || gate->mType == oc::GateType::Or) {
            ...
            } else if (gate->mType == oc::GateType::Xor || gate->mType == oc::GateType::Nxor) {
                for (u64 i = 0; i < out.size(); ++i)
                    out[i] = in[0][i] ^ in[1][i];
        ...
        for (gate = gates.begin(); gate < gates.end(); ++gate) {
            ...
            if (gate->mType == oc::GateType::na_And || gate->mType == oc::GateType::nb_And || gate->mType == oc::GateType::And ||
                gate->mType == oc::GateType::Or || gate->mType == oc::GateType::Nor) {
        ...
        if (mO.mDebug) {
            print = [&](u64 gIdx) {
                while (mDebugPrintIdx < mN && mPrint != mCir.mPrints.end() && mPrint->mGateIdx <= gIdx && mIdx) {
