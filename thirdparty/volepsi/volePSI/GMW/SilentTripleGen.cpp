...
namespace volePSI {
    ...
    Proto generateBase(
        RequiredBase b, u64 partyIdx, oc::PRNG &prng, coproto::Socket &chl, span<block> recvMsgP, span<std::array<block, 2>> sendMsgP, oc::Timer *timer)
    {
    ...
    Proto extend(
        RequiredBase b, span<std::array<block, 2>> baseMsg, oc::PRNG &prng, coproto::Socket &chl, span<block> recvMsgP, span<std::array<block, 2>> sendMsgP)
    {
        auto recvMsg = std::vector<block>{};
        auto sendMsg = std::vector<std::array<block, 2>>{};
        ...
        if (recvMsgP.size() != b.mRecvChoiceBits.size() || sendMsgP.size() != b.mNumSend)
            throw RTE_LOC;
